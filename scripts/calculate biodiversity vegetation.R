...
Biodiv_vegetation <- data.frame(
  ...
  NeophyteHerb.prop = rowSums(vegcomm[ ,which(species_data$status_category_Birgit == "N" & 
            species_data$LifeForm != "Phanerophyte" )]>0)/rowSums(vegcomm>0),
  Neophyte.prop = rowSums(vegcomm[ ,which(species_data$status_category_Birgit == "N")]>0)/rowSums(vegcomm>0),
